...
export const ApplyPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const errorMsg = err.response?.data?.error || err.response?.data?.message || 'Could not load application.';
      setError(errorMsg);
      console.error(err);
    } finally {
      setLoading(false);
      ...
      return (
        <div className="min-h-screen bg-portal-bg flex items-center justify-center p-6">
          <div className="max-w-md w-full bg-portal-card border border-portal-border rounded-3xl p-8 text-center shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-portal-text">Application Unavailable</h3>
            <p className="text-portal-text-muted mt-2 mb-8">{error || 'This bootcamp is not accepting applications yet.'}</p>
            <Link to="/dashboard" className="block w-full py-4 bg-portal-accent text-portal-bg rounded-2xl font-bold hover:scale-[1.02] transition-all">Back to Dashboard</Link>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="min-h-screen bg-portal-bg flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-portal-card border border-portal-border rounded-3xl p-8 text-center shadow-2xl">
          ...
          <h3 className="text-xl font-bold text-portal-text">Authentication Required</h3>
          <p className="text-portal-text-muted mt-2 mb-8">You need an account to apply for <span className="text-portal-accent">{bootcamp?.name || 'this bootcamp'}</span>.</p>
          <div className="space-y-4">
            <Link to={`/signup?bootcampId=${bootcampId}`} className="block w-full py-4 bg-portal-accent text-portal-bg rounded-2xl font-bold hover:scale-[1.02] transition-all">Create Account & Apply</Link>
            <Link to={`/login?redirect=/apply/${bootcampId}`} className="block w-full py-4 bg-white/5 border border-white/10 text-white rounded-2xl font-bold hover:bg-white/10 transition-all">Sign In</Link>
          </div>
        </div>
      </div>
    );
